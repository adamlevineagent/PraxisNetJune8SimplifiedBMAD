...
export default function RealProvingGroundDemoPage() {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/register`, {
        ...
        body: JSON.stringify({
          username,
          password,
          email: email || `${username}@demo.praxisnetwork.ai`
        })
      });
      ...
      setApiError('Registration failed. API server may not be running.');
      // Allow demo to continue with simulated data
  ...
  const startInterview = async () => {
    ...
    try {
      ...
      const initialMessage: Message = {
        id: '1',
        role: 'assistant',
        content: data.initialMessage || "What's got your professional attention these days?",
        timestamp: new Date(),
      };
      ...
      setApiError('Failed to connect to AI service. Using demo mode.');
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Update essence metrics if provided
      if (data.essenceMetrics) {
        setEssenceMetrics(data.essenceMetrics);
      } else {
  ...
  const completeInterview = async () => {
    try {
      // Complete conversation with API
      ...
      const data = await response.json();
      console.log('Professional essence extracted:', data.essence);
    } catch (error) {
          ...
          <Card className="p-8 max-w-md mx-auto">
            ...
            <div className="space-y-4">
              ...
              <Button
                ...
                className="w-full"
              >
                Create Account (Real API)
              </Button>
              
              <div className="text-sm text-text-secondary text-center">
                <p className="font-medium mb-2">🔴 REAL API MODE</p>
                <p>This demo attempts to connect to the real API.</p>
                <p>If the API is not running, it will fall back to demo mode.</p>
              </div>
            </div>
          </Card>
        );
          ...
          <Card className="p-6 max-w-3xl mx-auto">
            ...
            {apiError && (
              <div className="mb-4 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                <p className="text-sm text-yellow-500">{apiError}</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-2">
                ...
                <div className="mt-2 text-xs text-text-secondary">
                  ...
                  {authToken === 'demo-token' && ' | Demo Mode'}
                </div>
              </div>
    ...
    <div className="min-h-screen bg-background-primary p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold mb-2">Proving Ground 1 - Real API Demo</h1>
          <p className="text-lg text-text-secondary">Experience the complete onboarding flow with real AI</p>
          <div className="mt-4 w-full h-px bg-accent-primary"></div>
        </div>

        {/* API Status */}
        <div className="mb-4 text-center">
          <Badge variant={authToken === 'demo-token' ? 'secondary' : 'default'}>
            {authToken === 'demo-token' ? '⚠️ Demo Mode (API Unavailable)' : '✅ Connected to Real API'}
          </Badge>
        </div>
